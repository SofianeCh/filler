                ...
                if line[column] in 'ox':
                    ...
                    if self.map.map_canvas.itemcget(self.map.map_data[row, column], "fill") != p.color:
                        ...
                        if line[column].lower() == 'o':
                            self.result.score_p1.set(f"Map conquise = {int(p.score/(self.rows*self.columns)*100)}%")
                        else:
                            self.result.score_p2.set(f"Map conquise = {int(p.score/(self.rows*self.columns)*100)}%")
